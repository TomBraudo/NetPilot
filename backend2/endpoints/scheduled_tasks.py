...
def create_scheduled_task_endpoint():
    ...
    try:
        ...
        if not all([router_id, service, task, hour is not None, minute is not None]):
            return build_error_response("Missing required fields: router_id, service, task, hour, minute", 400, "MISSING_FIELDS", start_time)
        ...
        # Call service
        result, error = create_scheduled_task(user_id, router_id, service, task, params, hour, minute, days_of_week)
        if error:
