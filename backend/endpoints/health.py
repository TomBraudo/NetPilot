...
def monitoring_health():
    """Checks monitoring (nlbw) daemon status."""
    execution_start_time = time.time()
    try:
        from utils.daily_management import check_daily_backup_status
        from managers.router_connection_manager import RouterConnectionManager
        rcm = RouterConnectionManager()
        is_running, error = check_daily_backup_status(rcm)
        if error:
            return build_error_response(f"Monitoring check failed: {error}", 500, "MONITORING_CHECK_FAILED", execution_start_time)
        return build_success_response({"monitoring_running": bool(is_running)}, execution_start_time)
